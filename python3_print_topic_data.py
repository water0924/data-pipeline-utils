...
def PrintMessageData(bag_path,topic_name):
    ...
    for bag in all_bag:
      ...
      n=1
      bag=rosbag.Bag(bag)
      for topic, msg, t in bag.read_messages(topics=[topic_name],raw=True):
        data_to_debug = topic_proto_map[topic_name]
        data_to_debug.ParseFromString(strip_header(msg[1]))
      
        with open (print_data_name,"ab") as f:
          time_data=GetTimeString(t.to_time())+"\n"
          f.write("================第{}条msg================\n".format(n).encode())
          f.write(time_data.encode())
          f.write(str(data_to_debug).encode())
          n=n+1
...
# string_data 其实就是忽略python3的utf8反序列化，并切割rosbag数据的前面四个字节
class ChurchString(String):
...
def strip_header(raw_data):
    church_msg = ChurchString().deserialize(raw_data).data
    # print(church_msg, type(church_msg))
    # print(raw_data, type(raw_data))
    val = ord("$")
    if len(church_msg) < 4:
        print("invailed msg: too short")
        return None
    elif church_msg[0] != val or church_msg[1] != val or church_msg[2] != val or church_msg[3] != val:
        ...
        # 读取header_size_field 区间的值转为size
        header_size = struct.unpack("I", church_msg[prefix_size : prefix_size + header_size_field_size])[0]
        return church_msg[info_size + header_size:]
